...
若 Router 预设已被删除，启动器会将仍引用它的空白会话切换为官方 `standard`，恢复模式选择；迁移兼容新版 Web bundle 中的声明式预设和旧版预设目录。修复前备份会话日志到 `$DSH_HOME/backups/`，通过追加预设选择记录完成迁移，保留原日志；已有消息或已开始对话的会话不自动迁移。

启动器为尚未适配新版 Harness 的插件提供只读会话事件兼容接口，避免旧版 `session.events` 调用导致对话失败。会话删除插件通过侧边栏记录的实际 ID 定位会话，兼容未命名及同名会话；侧边栏补丁兼容上游新增的 `aria-description` 等无障碍属性，相关修复会在更新后自动重新应用。新版 Harness 下还会将 Turn Rewind 从已移除的 settingsScope 服务迁移到配置表单，设置与检查点管理卡片显示在插件页面。

对于旧版 `dsh-better-sidebar`，启动器会移除与新版列表插槽冲突的 `turnTail` 注册，保留原生侧栏文件预览，并修复当前会话订阅和侧聊、子任务跳转。侧栏尚未挂载时，文件和标签页打开请求会等待对应会话就绪；主界面及按需加载的编辑器等模块也会适配改名后的图标，保留原来的尺寸。插件兼容修复直接应用到已安装的 JavaScript，无需手动编译。
