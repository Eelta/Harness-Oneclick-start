...
If a Router preset has been deleted, blank sessions still referencing it are switched to the official `standard` preset so mode selection works again. Session logs are backed up under `$DSH_HOME/backups/` before appending a preset-selection event; existing records are preserved. Sessions containing user messages or started turns are not migrated automatically.
